...
class Heart(Compartment):
    ...
    def __init__(self, R=0., L=0., C=0., P1=0., Q2=0., P2=0., Q1=0.):
        ...
        self.C0 = self.C
        self.L0 = self.L
        self.freq = 0.1   # pulses per second
        self.amp = 0.0

    # same rhs as in Compartment class (unaltered), but modify the
    # diameter of the heart (and thus R, L, C) in each step
    def rhs(self, t, y):
        self.R = self.R0 * (1. + self.amp * sin(self.freq * pi * self.t))
        self.C = self.C0 * (1. + self.amp * cos(self.freq * pi * self.t))
        P1 = y[0]
        Q2 = y[1]  # valve
